(() => {
    ...
    const sceneInfo = [
        {
            ...
            values: {
                videoImageCount: 300,
                imageSequence: [0, 299],
                canvas_opactiy: [1, 0, { start: 0.9, end: 1 }],
        ...
        },
        {
            // 1
            type: 'normal',
            // heightNum: 5, // normal 에서는 필요 없음 브라우저 높이의 5배로 scrollHeight 세팅
            scrollHeight: 0,
            objs: {
        ...
        },
        {
            ...
            values: {
                videoImageCount: 960,
                imageSequence: [0, 959],
                canvas_opactiy_in: [0, 1, { start: 0, end: 0.1 }],
                canvas_opactiy_out: [1, 0, { start: 0.95, end: 1 }],
                ...
                pinB_opaity_in: [0, 1, { start: 0.5, end: 0.55 }],
                pinC_opaity_in: [0, 1, { start: 0.72, end: 0.77 }],
                pinB_opaity_out: [1, 0, { start: 0.58, end: 0.63 }],
                pinC_opaity_out: [1, 0, { start: 0.85, end: 0.9 }],
            },
        },
        {
            // 3
    ...
    function checkMenu(){
        if (yOffset > 44) {
            document.body.classList.add('local-nav-sticy');
        }else {
            document.body.classList.remove('local-nav-sticy');
        }
    }
    ...
    function playAnimation() {
        ...
        switch (currentScene) {
            case 0:
                ...
                objs.canvas.style.opacity = calcValues(values.canvas_opactiy, currentYOffset);
            ...
            case 2:
                ...
                if (scrollRatio <= 0.5) {
                    //in
                    objs.canvas.style.opacity = calcValues(values.canvas_opactiy_in, currentYOffset);
                } else {
                    //out
                    objs.canvas.style.opacity = calcValues(values.canvas_opactiy_out, currentYOffset);
                }
                ...
                if (scrollRatio > 0.9) {
                    ...
                    // 캔버스 사이즈에 맞춰 갖어한 innerWidth와 innerHeight
            ...
            case 3:
                // console.log('3 play');
                let step = 0;
                // 가로/세로 모두 꽉 차게 하기위해 여기서 셋팅 (계산필요)
                const widthRatio = window.innerWidth / objs.canvas.width;
                const heightRatio = window.innerHeight / objs.canvas.height;
                let canvasScaleRatio;
    ...
    // window.addEventListener('DOMContentLoaded', setLayout); // DOMContent 로드 후 함수 출력 (이미지 x)
